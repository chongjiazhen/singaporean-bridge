...
function TutorialOverlay({ onClose }: { onClose: () => void }) {
  const steps = [
    ...
    "Call a card: the declarer names a card they don't hold. Whoever has it becomes their partner. With hidden partner on (the default), only that player knows until the card is played. Until that card appears, only its holder knows the partnership: the declarer knows a partner exists but not which seat holds the card, and each defender plays without knowing which of the other two is a fellow defender and which is the declarer's secret partner. A declarer confident of winning alone may call a card they hold: they then have no partner and face all three, who may not realise until that card appears.",
